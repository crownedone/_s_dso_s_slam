...
FullSystem::FullSystem()
{
    ...
    ef = std::make_unique<EnergyFunctional>();
...
Vec4 FullSystem::trackNewCoarse(std::shared_ptr<FrameHessian> fh, std::shared_ptr<FrameHessian> fh_right)
{

    assert(allFrameHistory.size() > 0);
    // set pose initialization.

    //    LOG_INFO("the size of allFrameHistory is %d \n", (int)allFrameHistory.size());
    ...
    if (allFrameHistory.size() == 2)
    {
        ...
        {
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, rotDelta, 0, 0), Vec3(0, 0, 0)));         // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, rotDelta, 0), Vec3(0, 0, 0)));         // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, 0, rotDelta), Vec3(0, 0, 0)));         // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, 0, 0), Vec3(0, 0, 0)));            // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, -rotDelta, 0), Vec3(0, 0, 0)));            // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, 0, -rotDelta), Vec3(0, 0, 0)));            // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, rotDelta, rotDelta, 0), Vec3(0, 0, 0)));  // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, rotDelta, rotDelta), Vec3(0, 0, 0)));  // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, rotDelta, 0, rotDelta), Vec3(0, 0, 0)));  // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, rotDelta, 0), Vec3(0, 0, 0))); // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, -rotDelta, rotDelta), Vec3(0, 0, 0))); // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, 0, rotDelta), Vec3(0, 0, 0))); // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, rotDelta, -rotDelta, 0), Vec3(0, 0, 0))); // assume constant motion.
            ...
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, 0, -rotDelta, -rotDelta), Vec3(0, 0, 0)));    // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, 0, -rotDelta), Vec3(0, 0, 0)));    // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, -rotDelta, -rotDelta), Vec3(0, 0,
                                           0)));    // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, -rotDelta, rotDelta), Vec3(0, 0, 0))); // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, rotDelta, -rotDelta), Vec3(0, 0, 0))); // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, -rotDelta, rotDelta, rotDelta), Vec3(0, 0, 0)));  // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, rotDelta, -rotDelta, -rotDelta), Vec3(0, 0, 0))); // assume constant motion.
            lastF_2_fh_tries.push_back(SE3(Eigen::Quaterniond(1, rotDelta, -rotDelta, rotDelta), Vec3(0, 0, 0)));  // assume constant motion.
    ...
    else
    {
        ...
        {
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, rotDelta, 0, 0), Vec3(0, 0,
                                       0)));          // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, 0, rotDelta, 0), Vec3(0, 0,
                                       0)));          // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, 0, 0, rotDelta), Vec3(0, 0,
                                       0)));          // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, -rotDelta, 0, 0), Vec3(0, 0,
                                       0)));         // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, 0, -rotDelta, 0), Vec3(0, 0,
            ...
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, 0, 0, -rotDelta), Vec3(0, 0,
                                       0)));         // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, rotDelta, rotDelta, 0), Vec3(0, 0,
                                       0)));   // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, 0, rotDelta, rotDelta), Vec3(0, 0,
                                       0)));   // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, rotDelta, 0, rotDelta), Vec3(0, 0,
                                       0)));   // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, -rotDelta, rotDelta, 0), Vec3(0, 0,
                                       0)));  // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, 0, -rotDelta, rotDelta), Vec3(0, 0,
            ...
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, -rotDelta, 0, -rotDelta), Vec3(0, 0,
                                       0))); // assume constant motion.
            lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast * SE3(Eigen::Quaterniond(1, -rotDelta, -rotDelta, -rotDelta),
    ...
    for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++)
    {
        ...
        {
            LOG_INFO("take over. minRes %f -> %f!", achievedRes[0], coarseTracker->lastResiduals[0]);
    ...
    {
        LOG_INFO("Coarse Tracker tracked ab = %f %f (exp %f). Res %f!", aff_g2l.a, aff_g2l.b, fh->ab_exposure, achievedRes[0]);
    }
...
void FullSystem::stereoMatch(std::shared_ptr<ImageAndExposure> image,
                             ...
                             cv::Mat& idepthMap)
{
    ...
    for (ImmaturePoint* ph : fh->immaturePoints)
    {
        ...
        ImmaturePointStatus phTraceRightStatus = ph->traceStereo(fh_right->dI_ptr, K, 1);

        if (phTraceRightStatus == ImmaturePointStatus::IPS_GOOD)
        {
            ImmaturePoint* phRight = new ImmaturePoint(ph->lastTraceUV(0), ph->lastTraceUV(1), fh_right.get(), &Hcalib);
            ...
            ImmaturePointStatus  phTraceLeftStatus = phRight->traceStereo(fh->dI_ptr, K, 0);
...
void FullSystem::traceNewCoarse(std::shared_ptr<FrameHessian> fh)
{
    ...
    for (auto& host : frameHessians)        // go through all active frames
    {
        ...
        {
            ph->traceOn(fh.get(), KRKi, Kt, aff, &Hcalib, false);
    ...
    //  LOG_ERROR("ADD: TRACE: %'d points. %'d (%.0f%%) good. %'d (%.0f%%) skip. %'d (%.0f%%) badcond. %'d (%.0f%%) oob. %'d (%.0f%%) out. %'d (%.0f%%) uninit.\n",
    //          trace_total,
...
void FullSystem::traceNewCoarseNonKey(std::shared_ptr<FrameHessian> fh, std::shared_ptr<FrameHessian> fh_right)
{
    ...
    for (auto& host : frameHessians)        // go through all active frames
    {
        ...
        for (ImmaturePoint* ph : host->immaturePoints)
        {
            ...
            ImmaturePointStatus phTrackStatus = ph->traceOn(fh.get(), KRKi, Kt, aff, &Hcalib, false);

            if (phTrackStatus == ImmaturePointStatus::IPS_GOOD)
            {
                ImmaturePoint* phNonKey = new ImmaturePoint(ph->lastTraceUV(0), ph->lastTraceUV(1), fh.get(), &Hcalib);
                ...
                ImmaturePointStatus phNonKeyStereoStatus = phNonKey->traceStereo(fh_right->dI_ptr, K, 1);

                if (phNonKeyStereoStatus == ImmaturePointStatus::IPS_GOOD)
                {
                    ImmaturePoint* phNonKeyRight = new ImmaturePoint(phNonKey->lastTraceUV(0), phNonKey->lastTraceUV(1), fh_right.get(), &Hcalib);
                    ...
                    ImmaturePointStatus  phNonKeyRightStereoStatus = phNonKeyRight->traceStereo(fh->dI_ptr, K, 0);
...
void FullSystem::activatePointsMT()
{
    ...
    auto newestHs = frameHessians.back();
            ...
            else if (ph->isOOB(fhsToKeepPoints, fhsToMargPoints) || host->flaggedForMarginalization)
            {
                ...
                if (ph->isInlierNew())
                {
                    ...
                    for (PointFrameResidual* r : ph->residuals)
                    {
                        ...
                        {
                            r->efResidual->fixLinearizationF(ef.get());
                            ngoodRes++;
                        }
                    }
                ...
                else
                {
                    ...
                    //LOG_INFO("drop point in frame %d (%d goodRes, %d activeRes)", ph->host->idx, ph->numGoodResiduals, (int)ph->residuals.size());
                }
...
void FullSystem::addActiveFrame(std::shared_ptr<ImageAndExposure> image, std::shared_ptr<ImageAndExposure> image_right, int id)
{
...
void FullSystem::addActiveFrame(std::shared_ptr<ImageAndExposure> image, int id)
{
    ...
    if (!initialized)
    {
        ...
        else if (coarseInitializer->trackFrame(fh, fh, outputWrapper))  // if SNAPPED
        {
    ...
    else    // do front-end operation.
    {
        ...
        if (!std::isfinite((double)tres[0]) || !std::isfinite((double)tres[1]) || !std::isfinite((double)tres[2]) ||
            !std::isfinite((double)tres[3]))
        {
            LOG_ERROR("Initial Tracking failed: LOST!\n");
            isLost = true;
            return;
        }
...
void FullSystem::mappingLoop()
{
    ...
    while (runMapping)
    {
        ...
        auto fh = unmappedTrackedFrames.front();
        unmappedTrackedFrames.pop_front();
        auto fh_right = unmappedTrackedFrames_right.front();
...
void FullSystem::makeKeyFrame(std::shared_ptr<FrameHessian> fh, std::shared_ptr<FrameHessian> fh_right)
{
    ...
    for (auto fh1 : frameHessians)     // go through all active frames
    {
        ...
        {
            PointFrameResidual* r = new PointFrameResidual(ph, fh1.get(), fh.get());
            r->setState(ResState::INP);
            ph->residuals.push_back(r);
            ef->insertResidual(r);
            ph->lastResiduals[1] = ph->lastResiduals[0];
            ph->lastResiduals[0] = std::pair<PointFrameResidual*, ResState>(r, ResState::INP);
            numFwdResAdde += 1;
        }
    }
    ...
    //LOG_INFO("allKeyFramesHistory size is %d ", (int)allKeyFramesHistory.size());
    LOG_INFO("rmse is %f ", rmse);
...
void FullSystem::makeKeyFrame(std::shared_ptr<FrameHessian> fh)
{
    ...
    if (allKeyFramesHistory.size() <= 4)
    {
        if (allKeyFramesHistory.size() == 2 && rmse > 20 * benchmark_initializerSlackFactor)
        {
            LOG_ERROR("I THINK INITIALIZATINO FAILED! Resetting.\n");
            initFailed = true;
        }
...
void FullSystem::initializeFromInitializer(std::shared_ptr<FrameHessian> newFrame)
{
    ...
    for (int i = 0; i < coarseInitializer->numPoints[0]; i++)
    {
        ...
        ImmaturePoint* pt = new ImmaturePoint(point->u + 0.5f, point->v + 0.5f, firstFrame.get(), point->my_type, &Hcalib);
...
void FullSystem::initializeFromInitializer(std::shared_ptr<FrameHessian> newFrame, std::shared_ptr<FrameHessian> newFrameR)
{
    boost::unique_lock<boost::mutex> lock(mapMutex);
    ...
    if (!setting_debugout_runquiet)
        LOG_INFO("Initialization: keep %.1f%% (need %d, have %d)!", 100 * keepPercentage,
                 (int)(setting_desiredPointDensity), coarseInitializer->numPoints[0]);
    ...
    for (int i = 0; i < coarseInitializer->numPoints[0]; i++)
    {
        ...
        pt->traceStereo(firstFrameRight->dI_ptr, K, 1);
    ...
    initialized = true;
    LOG_INFO("INITIALIZE FROM INITIALIZER (%d pts)!", (int)firstFrame->pointHessians.size());
}

void FullSystem::makeNewTraces(std::shared_ptr<FrameHessian> newFrame,
                               std::shared_ptr<FrameHessian> newFrameRight,
                               float* gtDepth)
{
    ...
    for (int y = patternPadding + 1; y < hG[0] - patternPadding - 2; y++)
        for (int x = patternPadding + 1; x < wG[0] - patternPadding - 2; x++)
        {
            ...
            ImmaturePoint* impt = new ImmaturePoint(x, y, newFrame.get(), selectionMap[i], &Hcalib);
    ...
    LOG_INFO("MADE %d IMMATURE POINTS!", (int)newFrame->immaturePoints.size());
...
void FullSystem::printLogLine()
{
    ...
    if (!std::isfinite(statistics_lastFineTrackRMSE))
    {
        statistics_lastFineTrackRMSE = 0;
    }

    if (!setting_debugout_runquiet )
        LOG_INFO("LOG %d: %f fine. Res: %d A, %d L, %d M; (%d / %d) forceDrop. a=%f, b=%f. Window %d (%d)",
