...
PointFrameResidual::PointFrameResidual(PointHessian* point_, FrameHessian* host_,
...
void PointFrameResidual::debugPlot()
{
    ...
    for(int i = 0; i < patternNum; i++)
    {
        ...
        {
            target->debugImage.at<cv::Vec3b>(static_cast<int>(projectedTo[i][1] + 0.5f),
                                             static_cast<int>(projectedTo[i][0] + 0.5f)) = cT;
        }
    }
}
...
void PointFrameResidual::applyRes(bool copyJacobians)
{
    if(copyJacobians)
    {
        ...
        if(state_NewState == ResState::INP)// && )
        {
