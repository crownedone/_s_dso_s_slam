        ...
        if(fixLinearization)
        {
            ...
            if(r->efResidual->isActive())
            {
                if(r->isNew)
                {
                    ...
                    Vec3f ptp_inf = r->host->targetPrecalc[r->target->idx].PRE_KRKiTll * Vec3f(p->u, p->v,
...
void FullSystem::setNewFrameEnergyTH()
{
    ...
    newFrame->frameEnergyTH = 26.0f * setting_frameEnergyTHConstWeight + newFrame->frameEnergyTH *
                              (1 - setting_frameEnergyTHConstWeight);
...
//  LOG_INFO("EnergyTH: mean %f, median %f, result %f (in %d, out %d)! \n",
...
Vec3 FullSystem::linearizeAll(bool fixLinearization)
{
    ...
    if(multiThreading)
    {
        treadReduce.reduce(boost::bind(&FullSystem::linearizeAll_Reductor, this, fixLinearization, toRemove,
                                       _1, _2, _3, _4), 0, activeResiduals.size(), 0);
    ...
    if(fixLinearization)
    {
        ...
        //LOG_INFO("FINAL LINEARIZATION: removed %d / %d residuals!\n", nResRemoved, (int)activeResiduals.size());
...
bool FullSystem::doStepFromBackup(float stepfacC, float stepfacT, float stepfacR, float stepfacA,
                                  float stepfacD)
{
...
//  meanStepC += Hcalib.step.norm();
    ...
    if(!setting_debugout_runquiet)
        LOG_INFO("STEPS: A %.1f; B %.1f; R %.1f; T %.1f. \t",
...
void FullSystem::printOptRes(const Vec3& res, double resL, double resM, double resPrior,
                             double LExact, float a, float b)
{
    LOG_INFO("A(%f)=(AV %.3f). Num: A(%d) + M(%d); ab %f %f!\n",
             res[0],
             std::sqrtf((float)(res[0] / (patternNum * ef->resInA + 0.00001))),
             ef->resInA,
             ef->resInM,
             a,
             b
...
float FullSystem::optimize(int mnumOptIts)
{
    ...
    for(int iteration = 0; iteration < mnumOptIts; iteration++)
    {
        ...
        double incDirChange = (1e-20 + previousX.dot(ef->lastX)) / (1e-20 + previousX.norm() *
                              ef->lastX.norm());
        previousX = ef->lastX;
        ...
        if(!setting_debugout_runquiet)
        {
            LOG_INFO("%s %d (L %.2f, dir %.2f, ss %.1f): \t",
    ...
    {
        LOG_INFO("KF Tracking failed: LOST!\n");
        isLost = true;
    }
