...
int main(int argc, char** argv)
{
    ...
    string strAssociationFilename = ass; // string(argv[4]);
    ...
    ORB_SLAM2::System SLAM(voc, set, ORB_SLAM2::System::RGBD, true);
    ...
    for(int ni = 0; ni < nImages; ni++)
    {
        ...
        imRGB = cv::imread(seq + "/" + vstrImageFilenamesRGB[ni],
                           cv::ImreadModes::IMREAD_UNCHANGED);
        imD = cv::imread(seq + "/" + vstrImageFilenamesD[ni],
                         cv::ImreadModes::IMREAD_UNCHANGED);
        ...
        if(imRGB.empty())
        {
            cerr << endl << "Failed to load image at: "
                 << seq << "/" << vstrImageFilenamesRGB[ni] << endl;
            return 1;
        }
