...
public:
    ...
    inline void update(const Eigen::Matrix<float, i, 1>& L, const Eigen::Matrix<float, j, 1>& R,
                       float w)
    {
        A += w * L * R.transpose();
        numIn1++;
...
public:
    ...
    {
        shiftUp(true);
        num = static_cast<size_t>(numIn1 + numIn1k + numIn1m);
    }
