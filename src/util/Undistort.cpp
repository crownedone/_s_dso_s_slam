    ...
    int w_, int h_)
{
    ...
    {
        LOG_INFO("NO PHOTOMETRIC Calibration!\n");
    }


    // read G.
    std::ifstream f(file.c_str());
    LOG_INFO("Reading Photometric Calibration from file %s\n", file.c_str());

    if (!f.good())
    {
        LOG_INFO("PhotometricUndistorter: Could not open file!\n");
        return;
    }
...

    {
        ...
        if(GDepth < 256)
        {
            LOG_INFO("PhotometricUndistorter: invalid format! got %d entries in first line, expected at least 256!\n",
                     (int)Gvec.size());
            return;
        }
        ...
        for(int i = 0; i < GDepth - 1; i++)
        {
            if(G[i + 1] <= G[i])
            {
                LOG_INFO("PhotometricUndistorter: G invalid! it has to be strictly increasing, but it isnt!\n");
                return;
            }
        }
...
{
    LOG_INFO("Reading Calibration from file %s", configFilename.c_str());
    ...
    {
        LOG_INFO("found RadTan (OpenCV) camera model, building rectifier.\n");
                        ...
                        &ic[0], &ic[1], &ic[2], &ic[3], &ic[4]) == 5)
    {
        if(ic[4] == 0)
        {
            LOG_INFO("found PINHOLE camera model, building rectifier.\n");
        ...
        {
            LOG_INFO("found ATAN camera model, building rectifier.\n");
    ...
    {
        LOG_INFO("could not read calib file! exit.");
        exit(1);
    }
...
void Undistort::loadPhotometricCalibration(std::string file, std::string noiseImage,
                                           std::string vignetteImage)
{
...
{
    LOG_INFO("finding CROP optimal new model!\n");
    K.setIdentity();
    ...
    LOG_INFO("initial range: x: %.4f - %.4f; y: %.4f - %.4f!\n", minX, maxX, minY, maxY);
    ...
    while(oobLeft || oobRight || oobTop || oobBottom)
    {
        ...
        LOG_INFO("iteration %05d: range: x: %.4f - %.4f; y: %.4f - %.4f!\n", iteration,  minX, maxX, minY,
                 maxY);

        if(iteration > 500)
        {
            LOG_INFO("FAILED TO COMPUTE GOOD CAMERA MATRIX - SOMETHING IS SERIOUSLY WRONG. ABORTING \n");
            exit(1);
        }
    }
...
void Undistort::readFromFile(const char* configFileName, int nPars, std::string prefix)
{
    ...
    if(nPars == 5) // fov model
    {
        ...
        if(std::sscanf(l1.c_str(), buf, &parsOrg[0], &parsOrg[1], &parsOrg[2], &parsOrg[3],
                       &parsOrg[4]) == 5 &&
           std::sscanf(l2.c_str(), "%d %d", &wOrg, &hOrg) == 2)
        {
            LOG_INFO("Input resolution: %d %d\n", wOrg, hOrg);
            LOG_INFO("In: %f %f %f %f %f\n",
                     parsOrg[0], parsOrg[1], parsOrg[2], parsOrg[3], parsOrg[4]);
        }
        else
        {
            LOG_INFO("Failed to read camera calibration (invalid format?)\nCalibration file: %s\n",
                     configFileName);
            infile.close();
            return;
        }
    ...
    {
        LOG_INFO("called with invalid number of parameters.... forgot to implement me?\n");
        infile.close();
        return;
    }
    ...
    if(l3 == "crop")
    {
        ...
        LOG_INFO("Out: Rectify Crop\n");
    }
    else if(l3 == "full")
    {
        outputCalibration[0] = -2;
        LOG_INFO("Out: Rectify Full\n");
    }
    else if(l3 == "none")
    {
        outputCalibration[0] = -3;
        LOG_INFO("Out: No Rectification\n");
    }
    ...
    {
        LOG_INFO("Out: Failed to Read Output pars... not rectifying.\n");
        infile.close();
        return;
    }
    ...
    if(std::sscanf(l4.c_str(), "%d %d", &w, &h) == 2)
    {
        ...
        LOG_INFO("Output resolution: %d %d\n", w, h);
    }
    else
    {
        LOG_INFO("Out: Failed to Read Output resolution... not rectifying.\n");
        valid = false;
    }
    ...
    else if(outputCalibration[0] == -3)
    {
        if(w != wOrg || h != hOrg)
        {
            LOG_ERROR("rectification mode none requires input and output dimenstions to match!\n\n");
            exit(1);
        }
    ...
    LOG_INFO("%.1f %.1f %.1f", K(1, 0), K(1, 1), K(1, 2));
    LOG_INFO("%.1f %.1f %.1f", K(2, 0), K(2, 1), K(2, 2));
}


UndistortFOV::UndistortFOV(const char* configFileName, bool noprefix)
{
    LOG_INFO("Creating FOV undistorter\n");
...
void UndistortFOV::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y,
                                      int n) const
{
...
{
    LOG_INFO("Creating RadTan undistorter\n");
...
void UndistortRadTan::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y,
                                         int n) const
{
    // RADTAN
    float fx = parsOrg[0];
...
{
    LOG_INFO("Creating Equidistant undistorter\n");
...
void UndistortEquidistant::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y,
                                              int n) const
{
    // EQUI
    float fx = parsOrg[0];
...
{
    LOG_INFO("Creating KannalaBrandt undistorter\n");
...
void UndistortPinhole::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y,
                                          int n) const
{
    // current camera parameters
